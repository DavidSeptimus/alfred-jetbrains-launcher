...
| Custom task terminal  | `JB_TASK_TERMINAL_CMD` | _(none)_                         | Launch tasks in any terminal: `{cmd}` / `{cwd}` / `{name}` (e.g. `kitty @ launch --type=tab --cwd {cwd} {cmd}`). Overrides the built-in                                     |
